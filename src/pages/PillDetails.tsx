...
const pillDetailsData = {
  1: { name: 'Vitamin D3', pillsLeft: 28, lastTaken: '8:00 AM', nextIntake: '8:00 AM', scheduledDoses: 1 },
  2: { name: 'Omega-3', pillsLeft: 15, lastTaken: '12:30 PM', nextIntake: '12:30 PM', scheduledDoses: 2 },
  3: { name: 'Magnesium', pillsLeft: 4, lastTaken: '9:00 PM', nextIntake: '9:00 PM', scheduledDoses: 1 },
  4: { name: 'B-Complex', pillsLeft: 22, lastTaken: '8:00 AM', nextIntake: 'Tomorrow 8:00 AM', scheduledDoses: 1 },
  5: { name: 'Calcium', pillsLeft: 18, lastTaken: '7:00 PM', nextIntake: '7:00 PM', scheduledDoses: 3 },
  6: { name: 'Iron', pillsLeft: 2, lastTaken: '1:00 PM', nextIntake: 'Tomorrow 1:00 PM', scheduledDoses: 1 },
  7: { name: 'Zinc', pillsLeft: 35, lastTaken: '10:00 AM', nextIntake: 'Tomorrow 10:00 AM', scheduledDoses: 1 },
  8: { name: 'Probiotic', pillsLeft: 12, lastTaken: '6:00 AM', nextIntake: 'Tomorrow 6:00 AM', scheduledDoses: 2 }
};

// Generate dummy chart data for individual pill
const generateChartData = (scheduledDoses: number): PillChartData[] => [
  { day: 'Mon', date: '15', pillsTaken: Math.min(scheduledDoses, Math.floor(Math.random() * scheduledDoses) + 1), totalScheduled: scheduledDoses },
  { day: 'Tue', date: '16', pillsTaken: Math.min(scheduledDoses, Math.floor(Math.random() * scheduledDoses) + 1), totalScheduled: scheduledDoses },
  { day: 'Wed', date: '17', pillsTaken: scheduledDoses, totalScheduled: scheduledDoses },
  { day: 'Thu', date: '18', pillsTaken: Math.min(scheduledDoses, Math.floor(Math.random() * scheduledDoses) + 1), totalScheduled: scheduledDoses },
  { day: 'Fri', date: '19', pillsTaken: Math.min(scheduledDoses, Math.floor(Math.random() * scheduledDoses) + 1), totalScheduled: scheduledDoses },
  { day: 'Sat', date: '20', pillsTaken: Math.min(scheduledDoses, Math.floor(Math.random() * scheduledDoses) + 1), totalScheduled: scheduledDoses },
  { day: 'Sun', date: '21', pillsTaken: scheduledDoses, totalScheduled: scheduledDoses }
];
      ...
      <section className="fixed top-16 left-0 right-0 h-80 bg-background border-b border-border z-40">
        <div className="container mx-auto px-6 py-6 h-full">
          <div className="bg-card rounded-lg p-6 h-full border border-border">
            <h2 className="text-lg font-semibold text-foreground mb-4">7-Day Intake History</h2>
        ...
        <div className="container mx-auto px-6">
          <div className="bg-gradient-to-br from-primary/10 via-secondary/10 to-accent/10 rounded-xl p-6 border border-border/50 backdrop-blur-sm">
            ...
            <div className="flex items-center justify-between p-4 bg-card/30 rounded-lg border border-border/20 mb-6">
              ...
              <Switch 
                checked={dynamicSchedule} 
                onCheckedChange={setDynamicSchedule}
              />
            </div>
            ...
            <div className="space-y-4">
              ...
              {generateScheduleTimes(pillData.scheduledDoses).map((time, index) => (
                <div key={index} className="flex items-center justify-between p-4 bg-card/20 rounded-lg border border-border/10">
                  <div>
